...
import io.github.nexllm.console.model.request.CreateProviderKeyRequest;
import io.github.nexllm.console.model.request.PatchProviderKeyRequest;
import io.github.nexllm.console.model.response.ProviderKeyResponse;
import io.github.nexllm.console.model.response.ProviderKeyValueResponse;
...
public class ProviderKeyService {
    ...
    public List<ProviderKeyResponse> getProviderKeys(UUID providerId) {
        return providerKeyRepository.findByTenantId(SecurityContextUtils.getTenantId(), Sort.by(Order.desc("id")))
            .stream()
    ...
    private ProviderKeyResponse mapToResponse(ProviderKeyEntity entity) {
    ...
    public ProviderKeyResponse createProviderKey(@Valid CreateProviderKeyRequest createRequest) {
        if (providerKeyRepository.existsByTenantIdAndName(SecurityContextUtils.getTenantId(), createRequest.name())) {
            throw new BizException(ErrorCode.ADM_CONFLICT, createRequest.name());
        }
        ProviderKeyEntity providerKeyEntity = ProviderKeyEntity.builder()
    ...
    public void batchDelete(@NotNull List<UUID> ids) {
        providerKeyRepository.deleteByTenantIdAndProviderKeyIdIn(
            SecurityContextUtils.getTenantId(), ids);
    }

    public ProviderKeyResponse patchProviderKey(UUID providerKeyId,
        @Valid PatchProviderKeyRequest patchRequest) {
        ProviderKeyEntity providerKeyEntity = providerKeyRepository.findOneByTenantIdAndProviderKeyId(
            SecurityContextUtils.getTenantId(), providerKeyId);
        ...
        if (patchRequest.name() != null) {
            boolean exists = providerKeyRepository.existsByTenantIdAndIdNotAndName(SecurityContextUtils.getTenantId(),
    ...
    public ProviderKeyValueResponse getProviderKeyValue(UUID providerKeyId) {
        ProviderKeyEntity entity = providerKeyRepository.findOneByTenantIdAndProviderKeyId(
            SecurityContextUtils.getTenantId(), providerKeyId);
